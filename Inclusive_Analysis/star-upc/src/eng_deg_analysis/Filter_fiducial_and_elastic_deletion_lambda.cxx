...
int main(int argc, char** argv) 
{
    ...
    ROOT::TThreadedObject<TH1D> EafterfiducialFidHist("EafterfiducialFidHist", ";E_{p} [GeV];tracks", 200, 100, 300);
    //Energy after both cuts
    ROOT::TThreadedObject<TH1D> EafterallFidHist("EafterallFidHist", ";E_{p} [GeV];tracks", 200, 100, 300);
    //DCA
    ROOT::TThreadedObject<TH1D> DCAHist("DCAHist", ";DCA [cm];tracks", 100, 0, 3);
    //Vertex x,y,z,r
    ROOT::TThreadedObject<TH1D> verXfinHist("verXfinHist", ";Vertex x position [cm];events", 140, -0.4, 0.1);
    ROOT::TThreadedObject<TH1D> verYfinHist("verYfinHist", ";Vertex y position [cm];events", 100, -0.49, 0.01);
    ...
    auto myFunction = [&](TFile* myFile) {
        ...
        auto MLambdaforMatchFidHistLocal = MLambdaforMatchFidHist.Get();
        auto MLambdabcgforMatchFidHistLocal = MLambdabcgforMatchFidHist.Get();
        ...
        auto verXfinHistLocal = verXfinHist.Get();
        auto verYfinHistLocal = verYfinHist.Get();
        auto verRfinHistLocal = verRfinHist.Get();
        auto verZfinHistLocal = verZfinHist.Get();
        ...
        do{
            ...
            for (long unsigned int posi = 0; posi < posProton.size(); posi++){
                for (long unsigned int negi = 0; negi < negPion.size(); negi++)
                {
                    ...
                    MLambdaforMatchFidHistLocal->Fill(tempSum.M());
                    //specific
            ...
            for (long unsigned int posi = 0; posi < posPion.size(); posi++){
                for (long unsigned int negi = 0; negi < negPion.size(); negi++)
                {
                    if(posPion[posi].M()<0 || negPion[negi].M()<0){
                        continue;
                    }
            ...
            for (long unsigned int posi = 0; posi < posKaon.size(); posi++){
                for (long unsigned int posi2 = 0; posi2 < posi; posi2++)
                {
                    tempSum = posKaon[posi] + posKaon[posi2];
                    MbcgforThesisFidHistLocal->Fill(tempSum.M());
                    MbcgforMatchFidHistLocal->Fill(tempSum.M());
                    MLambdabcgforMatchFidHistLocal->Fill(tempSum.M());
                }
            ...
            for (long unsigned int negi = 0; negi < negKaon.size(); negi++){
                for (long unsigned int negi2 = 0; negi2 < negi; negi2++)
                {
                    tempSum = negKaon[negi] + negKaon[negi2];
                    MbcgforThesisFidHistLocal->Fill(tempSum.M());
                    MbcgforMatchFidHistLocal->Fill(tempSum.M());
                    MLambdabcgforMatchFidHistLocal->Fill(tempSum.M());
                }
            ...
            for (long unsigned int posi = 0; posi < posProton.size(); posi++){
                for (long unsigned int posi2 = 0; posi2 < posi; posi2++)
                {
                    tempSum = posProton[posi] + posProton[posi2];
                    MbcgforThesisFidHistLocal->Fill(tempSum.M());
                    MbcgforMatchFidHistLocal->Fill(tempSum.M());
                    MLambdabcgforMatchFidHistLocal->Fill(tempSum.M());
                }
            ...
            for (long unsigned int negi = 0; negi < negProton.size(); negi++){
                for (long unsigned int negi2 = 0; negi2 < negi; negi2++)
                {
                    tempSum = negProton[negi] + negProton[negi2];
                    MbcgforThesisFidHistLocal->Fill(tempSum.M());
                    MbcgforMatchFidHistLocal->Fill(tempSum.M());
                    MLambdabcgforMatchFidHistLocal->Fill(tempSum.M());
                }
            ...
            for (int veri = 0; veri < tempUPCpointer->getNumberOfTracks(); veri++)
            {
                if(tempUPCpointer->getTrack(veri)->getFlag(StUPCTrack::kPrimary)){
                    primVertex = tempUPCpointer->getTrack(veri)->getVertex();
                    break;
                }
            }
            
            verXfinHistLocal->Fill(primVertex->getPosX());
            verYfinHistLocal->Fill(primVertex->getPosY());
            verRfinHistLocal->Fill(sqrt(primVertex->getPosX()*primVertex->getPosX() + primVertex->getPosY()*primVertex->getPosY()));
            verZfinHistLocal->Fill(primVertex->getPosZ());
    ...
    auto MLambdaforMatchFidHistFinal = MLambdaforMatchFidHist.Merge();
    auto MLambdabcgforMatchFidHistFinal = MLambdabcgforMatchFidHist.Merge();
    ...
    auto verXfinHistFinal = verXfinHist.Merge();
    auto verYfinHistFinal = verYfinHist.Merge();
    auto verRfinHistFinal = verRfinHist.Merge();
    auto verZfinHistFinal = verZfinHist.Merge();
    ...
    //setting up a tree & output file
    string outfileName = outputFolder + "AnaOutput_FidHistForOverleafLambdaSearch.root";
    cout<<"Created output file "<<outfileName<<endl;
    ...
    MLambdaforMatchFidHistFinal->Write();
    MLambdabcgforMatchFidHistFinal->Write();
    EbeforeFidHistFinal->Write();
    EafterfiducialFidHistFinal->Write();
    EafterallFidHistFinal->Write();
    DCAHistFinal->Write();
    verXfinHistFinal->Write();
    verYfinHistFinal->Write();
    verRfinHistFinal->Write();
    verZfinHistFinal->Write();
