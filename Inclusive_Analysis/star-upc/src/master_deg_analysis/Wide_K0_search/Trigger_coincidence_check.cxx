...
int main(int argc, char **argv){
    ...
    auto myFunction = [&](TFile *myFile){
        ...
        do{
            ...
            if(tempUPCpointer->isTrigger(570701)&&tempUPCpointer->isTrigger(570704)&&tempUPCpointer->getRunNumber()<=18083025){
                coincident_entries++;
            }
            if(tempUPCpointer->isTrigger(570705)&&tempUPCpointer->isTrigger(570704)&&tempUPCpointer->getRunNumber()>18083025){
                coincident_entries++;
            }

        } while(myReader.Next());

        cout<<"Finished operation on input file "<<fileName<<" with "<<tempTree->GetEntries()<<" entries and "<<coincident_entries<<" weird entries"<<endl;
        tempFile->Close();

        return coincident_entries;
        };
