...
if [ "$#" -eq "0" ]; then
    echo Usage:
    echo ./centralDiffractive_total_chain_only_MuDST_start.sh [number of events] [run number \(optional, default 18091010\)] ["seed" \(optional, default 0\)]
fi
...
if [ "$full_events" -ne "0" ]; then
    ...
    do
        star-submit-template -template centralDiffractive_total_chain_only_MuDST_template.xml -entities number=$i,events_number=$n,run_number=$run_number,seed=$seed,output_directory=$output_directory
    done
fi

# doing last, not-full batch
if [ "$partial_events" -ne "0" ]; then
    star-submit-template -template centralDiffractive_total_chain_only_MuDST_template.xml -entities number=$full_events,events_number=$partial_events,run_number=$run_number,seed=$seed,output_directory=$output_directory
fi
