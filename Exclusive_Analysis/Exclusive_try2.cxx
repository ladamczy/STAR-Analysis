...
int main(int argc, char** argv)  
{
    ...
    for (Long64_t i = 0; i < chain->GetEntries(); ++i) 
    {
        ...
        {
            dcaBeamline = ExclusiveK0K0::DCA_BEAMLINE;
        }

        else
        {
            dcaBeamline = ExclusiveK0K0::DCA_BEAMLINE_23;
        }

        double dcaDaughters;
        if (tracksWithTofHit.size() == 4)
        {
            dcaDaughters = ExclusiveK0K0::DCA_BEAMLINE;
        }

        else
        {
            dcaDaughters = ExclusiveK0K0::DCA_BEAMLINE_23;
        }



        if (dcaDaughtersLeadingKaon <= dcaDaughters and dcaDaughtersSubLeadingKaon <= dcaDaughters)
        {
            areBothDcaDaughtersSmall = true;
        }

        if (DCABeamlineLeadingKaon <= dcaBeamline and DCABeamlineSubLeadingKaon <= dcaBeamline)
        {
        ...
        {
            if ((decayLengthLeadingKaon <= ExclusiveK0K0::DECAY_LENGTH or cosLeadingKaon >= ExclusiveK0K0::COS_PNT_ANG) and (decayLengthSubLeadingKaon <= ExclusiveK0K0::DECAY_LENGTH or cosSubLeadingKaon >= ExclusiveK0K0::COS_PNT_ANG))
            {
        ...
        {
            if ((decayLengthLeadingKaon <= ExclusiveK0K0::DECAY_LENGTH or cosLeadingKaon >= ExclusiveK0K0::COS_PNT_ANG) and (cosSubLeadingKaon >= ExclusiveK0K0::COS_PNT_ANG))
            {
                areBothPointingAngleOrDecayLength = true;
            }
        }
        ...
        {
            if (cosLeadingKaon >= ExclusiveK0K0::COS_PNT_ANG and cosSubLeadingKaon >= ExclusiveK0K0::COS_PNT_ANG)
            {
                areBothPointingAngleOrDecayLength = true;
            }
        }
        ...
        if ( abs(corrE) < 0.01 and abs(corrW) < 0.01)
        {
            areSeparateCorr = true;
        }

        if (abs(ksiCorrelation )<0.005)
        {
            isKsiCorrSmall = true;
        }

        if (abs(etaCorrelation) < 1.5)
        {
            isEtaCorrSmall = true;
        }
        ...
        if (N1_isPtMissingSmall==1) { HistPtMissN1[iH]->Fill(pTmiss);
        if (isPtMissingSmall)
