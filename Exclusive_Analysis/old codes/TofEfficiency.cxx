...
int main(int argc, char** argv)  
{
    ...
    for (Long64_t i = 0; i < chain->GetEntries(); ++i) 
    {
        ...
        for (Int_t i = 0; i<upcEvt->getNumberOfTracks(); i++)
		{
            if (isMC == 0)
            {
                if ( (abs(upcEvt->getTrack(i)->getEta()) <= 0.9) and (upcEvt->getTrack(i)->getPt() >= 0.2) and (upcEvt->getTrack(i)->getNhitsFit() >= 25) and upcEvt->getTrack(i)->getFlag(StUPCTrack::kV0) )
                {
            ...
            {
                if ( (abs(upcEvt->getTrack(i)->getEta()) <= 0.9) and (upcEvt->getTrack(i)->getPt() >= 0.2) and (upcEvt->getTrack(i)->getNhitsFit() >= 25)) 
                {
                    tracksWithSmallDca.push_back(upcEvt->getTrack(i));
        ...
        if (isMC==1) 
        {
            ...
            for (int j = 0; j < vPions.size(); j++) // pętla po wszyskich pionach z poziomu true - szukanie matchingu
            {
                ...
                if (abs(truthVertexZ) > 80 or abs(truthVertexR) > 3.0 or pT < 0.2)// sprawdzenie czy true vertex i pt pionu jest w odpowiednim zakresie 
                {
        ...
        for (int i = 0; i<tracksWithSmallDca.size(); i++) // pęta po trackach dobrej jakości
		{
            for (int j = 1; j<tracksWithSmallDca.size(); j++)
		    {
                if (i != j)
                {
                    ...
                    if ( kaon.dcaDaughters()<1.5 and   kaon.DCABeamLine()<1.5 and  (kaon.pointingAngleHypo()>0.925 )  )
                    {   
