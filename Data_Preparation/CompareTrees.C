...
void CompareTrees() {
    TFile* file1 = TFile::Open("treeparrentF.root");
    if (!file1 || file1->IsZombie()) {
        std::cerr << "Nie można otworzyć pliku treeparrentF.root." << std::endl;
        return;
    }

    TFile* file2 = TFile::Open("mergedTreeF.root");
    if (!file2 || file2->IsZombie()) {
        std::cerr << "Nie można otworzyć pliku mergedTreeF.root." << std::endl;
        return;
    }

    TTree* treeT = static_cast<TTree*>(file1->Get("T"));
    if (!treeT) {
        std::cerr << "Nie znaleziono drzewa o nazwie T w pliku treeparrentF.root." << std::endl;
        return;
    }

    TTree* treeT3 = static_cast<TTree*>(file2->Get("T3"));
    if (!treeT3) {
        std::cerr << "Nie znaleziono drzewa o nazwie T3 w pliku mergedTreeF.root." << std::endl;
        return;
    }
